...
foreach ($settings['items'] as $item) {
    ...
    $out .= '
        <div class="'.$cls.' item tbtr"><div class="inner tbtr">
...
if ($settings['disp'] == 'slider') {
    echo '<div data-xld =\'' . wp_kses_post(wp_json_encode($swiper_opt['settings'])) . '\' class="swiper tpswiper tp-img-card">
            <div class="swiper-wrapper">'.wp_kses_post($out).'</div>
            '.wp_kses_post($swiper_opt['nav']).'
        </div>'; 
} else {
    echo '<div class="tp-d-flex tp-gutter tp-img-card">'.wp_kses_post($out).'</div>';;
}
